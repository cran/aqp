#' @title Plot soil profiles below a dendrogram or cladogram
#' 
...
#' @param scaling.factor numeric, vertical scaling of the profile heights (default estimate printed if not specified)
#' @param width numeric, scaling of profile widths
#' @param y.offset numeric, vertical offset for top of profiles (default estimate printed if not specified)
#' @param dend.y.scale numeric, scaling of dendrogram (default estimate printed if not specified)
...
#' 
#' 
#' @note The default values of `scaling.factor`, `y.offset`, and `dend.y.scale` are based on `clust$height` and `max(x)`, and should be close to optimal. Use the defaults as a starting point for finer control.
#' 
...
#' @export
#'
plotProfileDendrogram <- function(
    x, 
    clust, 
  ...
  # dendrogram, this also inits plotting coordinates / extent
  plot(
    dend, 
    cex = 0.8, 
    direction = 'up', 
  ...
  # if not specified, print information about scaling factors etc.
  .msg <- c(
    ifelse(missing(scaling.factor), sprintf("scaling.factor: %s", signif(scaling.factor, 3)), NA),
    ifelse(missing(y.offset), sprintf("y.offset: %s", signif(y.offset, 3)), NA),
    ifelse(missing(dend.y.scale), sprintf("dend.y.scale: %s", signif(dend.y.scale, 3)), NA)
  )
  .msg <- na.omit(.msg)
